...
// x_pct/y_pct are now content-container-relative (% of max-w-3xl width / overlay height).
// VERSION 5 forces a cache-bust so old viewport-relative positions are discarded.
const PRESET_PINS = [
...
// Overlay: position:absolute inside the content container (max-w-3xl).
// Width matches the content container exactly → x_pct is content-relative on every screen.
// Height equals the full page height → y_pct is page-height-relative.
// No scroll transform needed — the overlay scrolls with the page.
...
export default function CommentPins({ page, showPresets = true, activeTab }) {
  // overlayRef replaces the old canvasRef + annotationCanvasRef.
  // It points to the single position:absolute overlay inside the content container.
  // getBoundingClientRect() on this ref gives the overlay's exact viewport position,
  // which we use for all coordinate calculations.
  ...
  const [presetPositions, setPresetPositions] = useState(() => {
    // VERSION 5 — cache-bust: old positions were viewport-relative, new ones are content-relative
    const VERSION = '5';
  ...
  const [annotationPos, setAnnotationPos] = useState(() => {
    // VERSION 3 — cache-bust: old position was viewport-relative
    const VERSION = '3';
  ...
  // overlayHeight = document scrollHeight, snapshotted at mount on the Design tab.
  // Used as the overlay's explicit height so y_pct% maps correctly to page positions.
  // Must NOT use a ResizeObserver — tab switches inflate scrollHeight and shift all pins.
  useLayoutEffect(() => {
  ...
  // Cursor broadcast — coordinates relative to the overlay (= content container).
  // x_pct: % of content width. y_pct: % of overlay height from its absolute top.
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      const overlayAbsTop = rect.top + window.scrollY;
      ch.send({ type: 'broadcast', event: 'cursor', payload: {
        id: sessionId,
        x_pct: ((e.clientX - rect.left) / rect.width) * 100,
        y_pct: overlayHeight > 0 ? ((e.pageY - overlayAbsTop) / overlayHeight) * 100 : 0,
        color: cursorColor,
      }});
    };
  ...
  // Main drag effect — annotation and visitor card drags.
  // getXY returns clientX (viewport-relative, no horizontal scroll) and pageY (absolute
  // page position). clientX is used for X; pageY minus the overlay's absolute top is used
  // for Y — this makes both coordinates content-container-relative and scroll-invariant.
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      const relY = pageY - overlayAbsTop;
      const maxPct = meta.isAnnotation ? 100 : 95;
      const x_pct = Math.max(0, Math.min(maxPct, ((relX - meta.offsetX_px) / rect.width) * 100));
      const y_pct = Math.max(0, Math.min(maxPct, ((relY - meta.offsetY_px) / overlayHeight) * 100));
  ...
  // Visitor self-service drags (sessionToken !== null) are not auth-dependent.
  useEffect(() => {
  ...
  const startAnnotationDrag = (e) => {
    ...
    dragMetaRef.current = {
      id: '__annotation__',
      isAnnotation: true,
      offsetX_px: (e.clientX - rect.left) - (annotationPos.x_pct / 100) * rect.width,
      offsetY_px: (e.pageY - overlayAbsTop)  - (annotationPos.y_pct / 100) * overlayHeight,
    };
  ...
  const startDrag = (e, id, x_pct, y_pct) => {
    ...
    dragStartRef.current  = { x: clientX, y: clientY }; // clientY for click-vs-drag detection
    ...
    dragMetaRef.current = {
      ...
      offsetX_px: (clientX - rect.left) - (x_pct / 100) * rect.width,
      offsetY_px: (pageY - overlayAbsTop) - (y_pct / 100) * overlayHeight,
    };
    dragPosRef.current = { x_pct, y_pct };
    setExpandedId(null);
    setDraggingId(id);
  ...
  // The overlay is position:absolute inside the content container (max-w-3xl, position:relative).
  // Its width always matches the content container width, making x_pct screen-size-independent.
  // The toolbar and login form are position:fixed — they float above the page from any DOM depth.
  return (
    <>
      <div
        ref={overlayRef}
      ...
      >
        {!hidden && (
          <>
            ...
            {/* Floating annotation — only on the home page */}
            {page === 'home' && (() => {
      ...
      {/* Toolbar — position:fixed, always visible regardless of DOM depth */}
